                        ...
                        {tutors && tutors.map((tutor, index) => {
                            ...
                            return (
                                <div key={index} className="my-5 w-3/6 flex flex-row">
                                    <img src={image} className="px-5 py-5 w-28 h-28"/>
                                    <div className="w-full">
                                        <div className="flex flex-row w-full items-center py-5 px-5">
                                            <h1 className={`text-2xl font-semibold mr-auto`}>{tutor[2].trim().substring(1, tutor[2].length - 2)}</h1>
                                            <p className="text-2xl font-bold">${tutor[1].trim()}</p>
                                        </div>
                                        <div className="px-5">{tutor[0].substring(3, tutor[0].length - 1)}</div>
                                    </div>
                                </div>
                            )
                        })}
